...
- 当前处于：Phase 0.2 和 Phase 1 已完成，准备进入 Phase 2 核心契约、配置、日志与存储。
- 上一文档基线提交：`f019040 文档：记录当前开发状态与下次启动提示词`
- 本轮 Phase 0.2 / Phase 1 变更以完成后的阶段提交为准。
- 当前主要进度来源：本文档和 `docs/agentproxy-development-plan.md`
- 当前代码状态：已初始化 TypeScript 工程骨架，包含 `package.json`、`tsconfig.json`、`src/`、Vitest、Biome、tsup/tsx 和 CLI 占位入口。
...
2. 阅读本文档，定位第一个未完成任务。
3. 从 Phase 2 开始，先实现核心领域类型、provider 契约和稳定错误码。
4. 保持 provider capability、metadata escape hatch 和 limited mode 设计边界。
...
当前项目状态是：Phase 0.2 实施前技术决策和 Phase 1 TypeScript 工程骨架已完成并提交，下一步从 Phase 2 开始。
请先运行 `git status --short` 和 `git log -1 --oneline` 核对最新提交与工作区状态。

请严格按照 docs/development-progress-tracker.zh.md 继续迭代，从第一个未完成项开始：
Phase 2 核心契约、配置、日志与 SQLite 存储。第一组建议只推进 Phase 2.1 核心领域类型和稳定错误码。
...
- [ ] 实现 `AgentProvider` 接口。
- [ ] 实现 `ProviderCapabilities`，包含 `schemaVersion`。
- [ ] 实现 `ProviderHealth`。
- [ ] 实现 `RuntimeHandle`。
- [ ] 实现 `AgentProxySession`。
- [ ] 实现 `ProviderSession`。
- [ ] 实现 `AgentEventEnvelope`。
- [ ] 实现核心事件 union。
- [ ] 实现稳定错误码。
- [ ] 实现 provider metadata escape hatch。

验收标准：

- [ ] 类型编译通过。
- [ ] capability 缺失字段默认视为 unsupported。
- [ ] provider 未知字段不会丢失。
...
- [ ] 实现结构化 logger。
- [ ] 为每次操作生成 `correlationId`。
- [ ] 日志字段包含 `providerId`、`runtimeId`、`sessionId`、`operation`。
- [ ] 实现 token、secret、password、api key、authorization 等脱敏。
- [ ] 确保 `--json` 输出不混入日志。
- [ ] debug 日志必须显式启用。

验收标准：

- [ ] env、config、args、error 的脱敏测试通过。
- [ ] human output 和 JSON output 分离正确。
