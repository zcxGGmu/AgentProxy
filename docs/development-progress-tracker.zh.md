...
- 最新 OpenCode Go 真实模型 smoke 修复/验证提交：本次 OpenCode Go smoke 提交；提交后以下次 `git log -1 --oneline` 为准。该修复不代表 Phase 6 TUI 已开始。
...
- 当前工作区预期：本次 OpenCode Go 真实模型 smoke 修复/验证提交后应保持干净；下次启动必须先运行 `git status --short` 和 `git log -1 --oneline` 复核最新提交。若最新提交是 OpenCode Go smoke 提交、post-Gate-5 run 兼容性修复提交、Gate 5 验证提交或后续文档同步提交，不要把它当成 Phase 6 TUI 进展；仍应从 Phase 6 AgentProxy TUI MVP 继续。
...
当前项目状态是：Phase 0.2、Phase 1、Phase 2、Gate 2、Phase 3、Gate 3、Phase 4.1-4.7、Gate 4、Phase 5.1 CLI Framework Foundation、Phase 5.2 Doctor CLI 工作流、Phase 5.3 Run Prompt Minimal Workflow、Phase 5.3 CLI Chat Native OpenCode Launcher Minimal Entry、Phase 5 Providers List/Inspect CLI、Phase 5 Runtime List CLI、Phase 5 Runtime Stop CLI、Phase 5 Sessions List/Show/Resume/Abort/Delete/Export/Import/Share/Unshare CLI、Phase 5 Config Get CLI、Phase 5 Config Set CLI 和 Gate 5 汇总验证已完成并验证。

当前最新 Phase 5 实现基线是 `b2cb5d6 阶段进展：完成 Phase 5 Config Set CLI`；最新 Gate 5 汇总验证提交以后续 `git log -1 --oneline` 显示的 Gate 5 验证提交为准；最新 Gate 4 汇总验证提交仍是 `549a979 阶段进展：完成 Gate 4 汇总验证`；最新 Phase 4 阶段实现提交是 `afdd3e0 阶段进展：完成 Phase 4.7 Provider Passthrough`。如果 `git log -1 --oneline` 显示 Gate 5 验证提交或后续文档同步提交，请不要把它当成新的 Phase 5 实现基线，继续以 `b2cb5d6` 作为 Phase 5 最新实现基线。
...
9. Phase 2.4、Phase 2.5、Phase 3.1、Phase 3.2、Phase 3.3、Phase 3.4、Phase 3.5、Phase 3 runtime 诊断/Gate 3、Phase 4.1 provider-layer health/capability、Phase 4.2 Model/Provider 列表、Phase 4.3 Session 同步、Phase 4.4 Session 创建与恢复、Phase 4.5 Message 发送与事件映射、Phase 4.6 Session 操作、Phase 4.7 Provider Passthrough、Gate 4 汇总验证、Phase 5.1 CLI Framework Foundation、Phase 5.2 Doctor CLI 工作流、Phase 5.3 `run [prompt]` 最小工作流、Phase 5.3 CLI `chat` native OpenCode launcher 最小入口、Phase 5 `providers list/inspect` CLI 最小工作流、Phase 5 `runtime list` CLI 最小工作流、Phase 5 `runtime stop` CLI 最小工作流、Phase 5 `sessions list` CLI 最小工作流、Phase 5 `sessions show` CLI 最小工作流、Phase 5 `sessions resume` CLI 最小工作流、Phase 5 `sessions abort` CLI 最小工作流、Phase 5 `sessions delete` CLI 最小工作流、Phase 5 `sessions export` CLI 最小工作流、Phase 5 `sessions import` CLI 最小工作流、Phase 5 `sessions share` CLI 最小工作流、Phase 5 `sessions unshare` CLI 最小工作流、Phase 5 `config get [key]` CLI 最小工作流、Phase 5 `config set <key> <value>` CLI MVP 和 Gate 5 汇总验证已完成，不要回退或扩展它们；下一步只处理 Phase 6 AgentProxy TUI MVP，不要重写 OpenCode runtime。
```
