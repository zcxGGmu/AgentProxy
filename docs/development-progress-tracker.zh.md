...
- 当前处于：Gate 4 汇总验证已通过；Phase 5.1 CLI Framework Foundation、Phase 5.2 `doctor` CLI 工作流、Phase 5.3 `run [prompt]` 最小工作流和 Phase 5.3 `chat` / native TUI launcher 最小入口已完成并验证；Phase 5 CLI MVP 其余真实业务命令尚未实现，完整 TUI 尚未进入。下一步应从 Phase 5 后续真实业务命令拆小推进，继续保持薄代理和控制面边界。
- 最新 Phase 5.2 实现提交：`c707f2c 阶段进展：完成 Phase 5.2 Doctor CLI 工作流`；若最新提交是后续文档同步提交，Phase 5 最新实现基线仍为该 Phase 5.2 实现提交。
- 最新 Phase 5.3 实现提交：`f2424eb 阶段进展：完成 Phase 5.3 Run Prompt Minimal Workflow`；若最新提交是后续文档同步提交，Phase 5 最新实现基线仍为该 Phase 5.3 实现提交。
...
- 当前启动基线：`f2424eb 阶段进展：完成 Phase 5.3 Run Prompt Minimal Workflow`；Gate 4 验证基线仍为 `549a979 阶段进展：完成 Gate 4 汇总验证`，最新 Phase 4 实现基线仍为 `afdd3e0 阶段进展：完成 Phase 4.7 Provider Passthrough`。
- 最新阶段实现提交：`f2424eb 阶段进展：完成 Phase 5.3 Run Prompt Minimal Workflow`；最新阶段门禁验证提交：`549a979 阶段进展：完成 Gate 4 汇总验证`。
...
| Phase 5 | 进行中 | Phase 5.1 CLI Framework Foundation、Phase 5.2 `doctor` CLI 工作流、Phase 5.3 `run [prompt]` 最小工作流和 Phase 5.3 `chat` / native TUI launcher 最小入口已完成并验证；CLI MVP 其余真实业务命令尚未实现，下一步是 Phase 5 后续命令拆小推进。 |
...
- [x] 完成 Phase 5.3 `chat` / native TUI launcher 最小入口，包含 `agentproxy chat` 调起 OpenCode 原生 TUI、workspace 参数传递、interactive stdio handoff、provider exit code 保留、child env allowlist、provider disabled/missing binary/invalid provider 稳定错误和 `--session` 显式 unsupported。

### 未完成

- [ ] Phase 5：除 Phase 5.1/5.2/5.3 的 `run` 与 `chat` native launcher 外，`sessions`、`providers inspect/list`、`runtime`、`config` 等 CLI MVP 工作流尚未实现。
...
2. 阅读本文档，定位第一个未完成任务。
3. 从 Phase 5 后续真实业务命令拆小继续；优先选择一个小范围 CLI MVP 剩余命令任务组，不要提前扩展到完整 TUI，也不要同时实现 `sessions`、`runtime`、`config` 等多个后续真实业务命令。
...
当前状态：Phase 5.1 的共享 CLI 框架与输出契约、Phase 5.2 `doctor` CLI 工作流和 Phase 5.3 `run [prompt]` 最小工作流已完成并验证。Commander 作为 CLI parser 继续保留；全局 flags、help、command routing、稳定退出码、`--json` 错误输出和 stdout/stderr 分工已收口；未实现 `chat`、`sessions`、`runtime`、`config` 真实业务命令或完整 TUI。
