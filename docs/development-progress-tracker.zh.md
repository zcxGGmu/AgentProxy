...
- [ ] Gate 1：TypeScript 工程骨架已初始化，基础校验通过。
...
- [ ] 添加 `test` 脚本。
- [ ] 添加 `lint` 脚本。
- [ ] 添加 `format` 或 `format:check` 脚本。
- [ ] 添加本地运行 CLI 的脚本。

验收标准：

- [ ] `pnpm build` 或等价命令可运行。
- [ ] `pnpm typecheck` 或等价命令通过。
- [ ] `pnpm test` 或等价命令通过。
- [ ] `pnpm lint` 或等价命令通过。

### 4.3 CLI 占位入口

- [ ] 实现 `agentproxy --help` 占位输出。
- [ ] 实现 `agentproxy --version` 占位输出。
- [ ] 保证 binary entry 可通过本地 dev 命令运行。

验收标准：

- [ ] 本地可执行 `agentproxy --help`。
- [ ] help 输出中列出后续核心命令。
