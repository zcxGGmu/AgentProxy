...
- [x] Confirm current git status and latest commit.
- [x] Review `tasks/lessons.md`, `docs/development-progress-tracker.zh.md`, `docs/agentproxy-development-plan.md`, and current `tasks/todo.md`.
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete Phase 5 implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 CLI MVP command surface marked as implemented but keep Gate 5 not passed until a separate summary validation completes.
...
- Initial working tree is clean.
- Latest commit is `b2cb5d6 阶段进展：完成 Phase 5 Config Set CLI`.
- Latest Phase 5 implementation baseline is `b2cb5d6 阶段进展：完成 Phase 5 Config Set CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.

Acceptance criteria:

- [x] The tracker clearly states Phase 5 CLI MVP command surface is implemented and verified, but Gate 5 has not passed.
- [x] Completed Phase 5 CLI workflows are listed explicitly through `config set <key> <value>`.
- [x] Remaining work is listed explicitly as Gate 5, Phase 6 AgentProxy TUI, and Phase 7-9.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from Gate 5 summary validation, not Phase 6 TUI.
- [x] Documentation verification passes with no source/test behavior changes.

Risks and constraints:

- Do not imply Gate 5 is complete before the summary validation pass is run and recorded.
- Do not treat `agentproxy chat` as Phase 6 AgentProxy TUI.
- Do not start implementing Gate 5 or Phase 6 in this documentation sync.
- Keep AgentProxy as a thin OpenCode control plane; v1 remains OpenCode-only.

Review notes:

- 2026-05-22: Documentation sync completed after confirming the latest implementation commit is `b2cb5d6 阶段进展：完成 Phase 5 Config Set CLI`. Updated `docs/development-progress-tracker.zh.md` so the latest Phase 5 implementation baseline, current startup baseline, completed/unfinished summary, Gate 5 status, Phase 6 not-started boundary, Phase 5 checklist wording, Review entry, and next-start prompt all point to the correct post-config-set state. Verification passed: `git status --short`, `git log -1 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.
...
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 marked in progress and Gate 5 not passed.
- [x] Keep remaining work limited to one small task group from `config`.
- [x] Keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification and create one detailed Chinese commit.
...
- Initial working tree is clean.
- Latest commit is `46fac9a 阶段进展：完成 Phase 5 Sessions Unshare CLI`.
- Latest Phase 5 implementation baseline is `46fac9a 阶段进展：完成 Phase 5 Sessions Unshare CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.

Acceptance criteria:

- [x] The tracker clearly states Phase 5 is not complete and Gate 5 has not passed.
- [x] Completed Phase 5 CLI workflows are listed explicitly through `sessions unshare`.
- [x] Remaining Phase 5 CLI MVP workflow is listed explicitly as `config`.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from exactly one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
- [x] Documentation verification passes with no source/test behavior changes.
...
- Do not imply Phase 5 or Gate 5 is complete.
- Do not treat `agentproxy chat` as Phase 6 AgentProxy TUI.
- Do not start implementing `config` in this documentation sync.
- Keep AgentProxy as a thin OpenCode control plane; v1 remains OpenCode-only.

Review notes:

- 2026-05-22: Documentation sync completed after confirming the latest implementation commit is `46fac9a 阶段进展：完成 Phase 5 Sessions Unshare CLI`. Updated `docs/development-progress-tracker.zh.md` so the latest Phase 5 implementation baseline, completed/unfinished summary, Gate 5 status, Phase 6 not-started boundary, Review entry, and next-start prompt all point to the correct post-unshare state. Verification passed: `git status --short`, `git log -1 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.

## Current Iteration - 2026-05-22 Phase 5 Sessions Unshare CLI Minimal Workflow

Scope: advance only the Phase 5 `agentproxy sessions unshare <id>` CLI task group. This workflow removes provider sharing from an existing visible AgentProxy local session mapping through the existing OpenCode provider session unshare API, updates only local share state metadata, and emits a transcript-free control-plane summary. Do not implement `config`, `chat --session`, Gate 5, Phase 6 AgentProxy TUI, generic provider passthrough expansion, or any new Agent runtime behavior.

Implementation checklist:

- [x] Confirm current git status and latest commit before implementation.
- [x] Review `tasks/lessons.md`, `docs/development-progress-tracker.zh.md`, `docs/agentproxy-development-plan.md`, and current `tasks/todo.md`.
- [x] Add focused CLI tests for `sessions unshare <id>` JSON/human output, provider unshare dispatch, local share metadata update to `shared=false`, absent/missing/tombstoned/wrong-provider/wrong-workspace records, disabled/invalid provider behavior, runtime unavailable behavior, provider failure behavior, terminal/control-character safety, no share URL persistence/output, and later-command placeholder boundaries.
- [x] Extend `src/cli/sessions.ts` with a narrow unshare action/service that resolves config, requires an existing visible local session mapping, opens SQLite read-write only after readonly visible mapping validation, reuses the OpenCode runtime/provider helper, calls existing `unshareAgentProxySession()`, and emits stable sanitized output.
- [x] Wire only `agentproxy sessions unshare <id>` from planned placeholder to a real action; keep `config`, `chat --session`, Gate 5, and Phase 6 TUI untouched.
- [x] Keep JSON/human output transcript-free and provider-payload-safe; do not print or persist share URLs, raw provider payloads, transcripts, metadata blobs, headers, or provider-controlled control characters.
- [x] Update `docs/development-progress-tracker.zh.md` completed/unfinished status and Review notes after verification.
...
- Initial working tree is clean; `git log -1 --oneline` is `e5bda79 文档：同步 Phase 5 Sessions Share 后最新状态`, so the latest Phase 5 implementation baseline remains `1c115a8 阶段进展：完成 Phase 5 Sessions Share CLI`.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.
- Phase 4.6 already provides `OpenCodeProvider.unshareSession()` and provider-agnostic `unshareAgentProxySession()` with local share metadata updates.
- Phase 5 `sessions share` already provides runtime selection, provider creation, visible local session mapping checks, sanitized JSON/human action output, and share URL non-persistence patterns to mirror.
...
- [x] `agentproxy sessions unshare <id>` is a real command and no longer returns the generic planned `CAPABILITY_UNSUPPORTED` placeholder.
...
- [x] The command calls the provider unshare operation for the existing providerSessionId, updates only local `sessionOperations.share.shared=false` metadata through the existing session operation service, and does not create, resume, delete, share, export, import, or send messages to a session.
- [x] `sessions unshare --json` emits exactly one valid redacted JSON object on stdout, with no human prose mixed in and no transcript/raw provider payload.
- [x] Human output is terminal-control-character safe and does not print raw metadata blobs, transcripts, provider event payloads, URL credentials, query-string secrets, share URLs, headers, export payloads, import sources, or provider-controlled control characters.
- [x] Provider failure does not mark the local session unshared; local share metadata changes only after provider unshare succeeds.
...
- [x] `config`, `chat --session`, Gate 5, and Phase 6 AgentProxy TUI remain unimplemented or explicitly unsupported.
- [x] Focused tests pass, followed by `pnpm run typecheck`, `pnpm run test`, `pnpm run lint`, `pnpm run format:check`, `pnpm run build`, and `git diff --check`.

Risks and constraints:

- Unshare should be a narrow provider-backed control-plane operation; do not invent generic sharing semantics or store provider share artifacts.
- Share URLs can grant access, so unshare output must not echo historical or provider-derived share URLs and must not make them durable.
- The operation is mutating but not destructive; do not require `--yes`, and do not open writable SQLite before validating the visible local mapping.
- Avoid touching Phase 6 TUI or implementing config in the same iteration.

Review notes:

- 2026-05-22: Implemented the Phase 5 `sessions unshare` CLI minimal workflow. Added focused failing tests first, then extended `src/cli/sessions.ts` and `src/cli/index.ts` so `agentproxy sessions unshare <id>` resolves config, first validates the visible local AgentProxy session mapping through readonly existing SQLite, then opens writable storage for the provider unshare mutation, reuses the OpenCode runtime/provider helper, calls existing provider-backed `unshareAgentProxySession()`, and emits stable transcript-free JSON/human summaries. The command does not print or persist share URLs, omits providerSessionId from unshare success output, writes only `sessionOperations.share.shared=false` plus timestamp to SQLite, leaves provider failure without local unshared metadata, and keeps `config`, `chat --session`, Gate 5, and Phase 6 TUI untouched. Security review flagged providerSessionId output and a concurrent remap share-state write risk; both were fixed and follow-up code/security reviews reported no findings. Verification passed: `pnpm exec vitest run tests/cli-sessions.test.ts --testTimeout=15000` (1 file, 59 tests), `pnpm exec vitest run tests/session-actions.test.ts --testTimeout=15000` (1 file, 7 tests), `pnpm exec vitest run tests/cli-sessions.test.ts tests/cli-help.test.ts tests/cli-chat.test.ts tests/cli-run.test.ts tests/cli-runtime.test.ts tests/cli-providers.test.ts --testTimeout=15000` (6 files, 105 tests), `pnpm exec vitest run tests/session-actions.test.ts tests/opencode-provider-session-actions.test.ts --testTimeout=15000` (2 files, 13 tests), `pnpm run typecheck`, `pnpm run lint`, `pnpm run format:check`, `pnpm run test` (29 files, 280 tests), and `pnpm run build`. Residual risk: real OpenCode unshare endpoint response shape still needs later smoke calibration; `config`, `chat --session`, Gate 5, and Phase 6 AgentProxy TUI remain unimplemented.

## Current Iteration - 2026-05-22 Documentation Sync After Phase 5 Sessions Share CLI

Scope: update only the project tracking documents after `1c115a8 阶段进展：完成 Phase 5 Sessions Share CLI` so the next Codex session can resume from the correct remaining Phase 5 CLI MVP task group. This is documentation-only. Do not change source code, tests, provider behavior, runtime behavior, CLI command behavior, or TUI behavior.
...
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 marked in progress and Gate 5 not passed.
- [x] Keep remaining work limited to one small task group from `sessions unshare` or `config`.
- [x] Keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification and create one detailed Chinese commit.
...
- Initial working tree is clean.
- Latest commit is `1c115a8 阶段进展：完成 Phase 5 Sessions Share CLI`.
- Latest Phase 5 implementation baseline is `1c115a8 阶段进展：完成 Phase 5 Sessions Share CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.

Acceptance criteria:

- [x] The tracker clearly states Phase 5 is not complete and Gate 5 has not passed.
- [x] Completed Phase 5 CLI workflows are listed explicitly through `sessions share`.
- [x] Remaining Phase 5 CLI MVP workflows are listed explicitly as `sessions unshare` and `config`.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from exactly one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
- [x] Documentation verification passes with no source/test behavior changes.
...
- 2026-05-22: Documentation sync completed after confirming the latest implementation commit is `1c115a8 阶段进展：完成 Phase 5 Sessions Share CLI`. Updated `docs/development-progress-tracker.zh.md` so the latest Phase 5 implementation baseline, completed/unfinished summary, Gate 5 status, Phase 6 not-started boundary, Review entry, and next-start prompt all point to the correct post-share state. Verification passed: `git status --short`, `git log -1 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.
...
- [x] Confirm current git status and latest commit.
- [x] Review `tasks/lessons.md`, `docs/development-progress-tracker.zh.md`, and `docs/agentproxy-development-plan.md`.
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 marked in progress and Gate 5 not passed.
- [x] Keep remaining work limited to one small task group from `sessions share/unshare` or `config`.
- [x] Keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification and create one detailed Chinese commit.
...
- Initial working tree is clean.
- Latest commit is `ff78e4d 阶段进展：完成 Phase 5 Sessions Import CLI`.
- Latest Phase 5 implementation baseline is `ff78e4d 阶段进展：完成 Phase 5 Sessions Import CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.

Acceptance criteria:

- [x] The tracker clearly states Phase 5 is not complete and Gate 5 has not passed.
- [x] Completed Phase 5 CLI workflows are listed explicitly through `sessions import`.
- [x] Remaining Phase 5 CLI MVP workflows are listed explicitly as `sessions share/unshare` and `config`.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from exactly one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
...
- 2026-05-22: Documentation sync completed after confirming the latest implementation commit is `ff78e4d 阶段进展：完成 Phase 5 Sessions Import CLI`. Updated `docs/development-progress-tracker.zh.md` so the latest Phase 5 implementation baseline, completed/unfinished summary, Gate 5 status, Phase 6 not-started boundary, Review entry, and next-start prompt all point to the correct post-import state. Verification passed: `git status --short`, `git log -1 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.
...
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 marked in progress and Gate 5 not passed.
- [x] Keep remaining work limited to one small task group from `sessions import/share/unshare` or `config`.
- [x] Keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification and create one detailed Chinese commit.
...
- Initial working tree is clean.
- Latest commit is `479ed89 阶段进展：完成 Phase 5 Sessions Export CLI`.
- Latest Phase 5 implementation baseline is `479ed89 阶段进展：完成 Phase 5 Sessions Export CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.

Acceptance criteria:

- [x] The tracker clearly states Phase 5 is not complete and Gate 5 has not passed.
- [x] Completed Phase 5 CLI workflows are listed explicitly through `sessions export`.
- [x] Remaining Phase 5 CLI MVP workflows are listed explicitly as `sessions import/share/unshare` and `config`.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from exactly one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
- [x] Documentation verification passes with no source/test changes.
...
- 2026-05-22: Documentation sync completed after confirming the latest implementation commit is `479ed89 阶段进展：完成 Phase 5 Sessions Export CLI`. Updated `docs/development-progress-tracker.zh.md` so the latest Phase 5 implementation baseline, completed/unfinished summary, Gate 5 status, Phase 6 not-started boundary, Review entry, and next-start prompt all point to the correct post-export state. Verification passed: `git status --short`, `git log -1 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.
...
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 marked in progress and Gate 5 not passed.
- [x] Keep remaining work limited to one small task group from `sessions delete/export/import/share/unshare` or `config`.
- [x] Keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification and create one detailed Chinese commit.
...
- Initial working tree is clean.
- Latest commit is `1b1a017 阶段进展：完成 Phase 5 Sessions Abort CLI`.
- Latest Phase 5 implementation baseline remains `1b1a017 阶段进展：完成 Phase 5 Sessions Abort CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.

Acceptance criteria:

- [x] The tracker clearly states Phase 5 is not complete and Gate 5 has not passed.
- [x] Completed Phase 5 CLI workflows are listed explicitly through `sessions abort`.
- [x] Remaining Phase 5 CLI MVP workflows are listed explicitly as `sessions delete/export/import/share/unshare` and `config`.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from exactly one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
- [x] Documentation verification passes with no source/test changes.
...
- 2026-05-21: Documentation sync completed after confirming the latest implementation commit is `1b1a017 阶段进展：完成 Phase 5 Sessions Abort CLI`. Updated `docs/development-progress-tracker.zh.md` so the latest Phase 5 implementation baseline, completed/unfinished summary, Gate 5 status, Phase 6 not-started boundary, Review entry, and next-start prompt all point to the correct post-abort state. Verification passed: `git status --short`, `git log -3 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.
...
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 marked in progress and Gate 5 not passed.
- [x] Keep remaining work limited to one small task group from `sessions abort/delete/export/import/share/unshare` or `config`.
- [x] Keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification and create one detailed Chinese commit.
...
- Initial working tree is clean.
- Latest commit is `fb827fe 阶段进展：完成 Phase 5 Runtime Stop CLI`.
- Latest Phase 5 implementation baseline remains `fb827fe 阶段进展：完成 Phase 5 Runtime Stop CLI` unless a later documentation-only commit is present.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.
...
- [x] Remaining Phase 5 CLI MVP workflows are listed explicitly as `sessions abort/delete/export/import/share/unshare` and `config`.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from exactly one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
- [x] Documentation verification passes with no source/test changes.
...
- 2026-05-21: Documentation sync completed after confirming the latest implementation commit is `fb827fe`. Updated `docs/development-progress-tracker.zh.md` to anchor Phase 5 runtime stop to the concrete `fb827fe` baseline, keep Phase 5 in progress, keep Gate 5 not passed, and keep only `sessions abort/delete/export/import/share/unshare` and `config` as remaining Phase 5 CLI MVP work. Verification passed: `git status --short`, `git log -1 --oneline`, `pnpm run format:check`, and `git diff --check`. Source code and tests were not changed.
...
- [x] Confirm current git status and latest commit.
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 remaining CLI MVP work limited to `sessions resume/abort/delete/export/import/share/unshare`, `runtime stop`, and `config`; keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification.
- [x] Commit the documentation sync with a detailed Chinese commit message.

Dependencies confirmed before implementation:

- Initial working tree is clean; `git log -1 --oneline` is `7849c56 阶段进展：完成 Phase 5 Sessions Show CLI`.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.
- Latest Phase 5 implementation baseline should now be `7849c56` unless a later documentation-only commit is on top.

Acceptance criteria:

- [x] The tracker explicitly states completed work through Phase 5 `sessions show`.
- [x] The tracker explicitly states Phase 5 is still in progress and `sessions resume/abort/delete/export/import/share/unshare`, `runtime stop`, and `config` remain unfinished.
...
- [x] The tracker uses concrete commit `7849c56` for the latest Phase 5 sessions show implementation baseline instead of a placeholder.
...
- 2026-05-21: Documentation sync completed after confirming a clean tree at `7849c56`. Updated `docs/development-progress-tracker.zh.md` to use concrete Phase 5 sessions show implementation baseline `7849c56`, kept Phase 5 remaining CLI MVP work limited to `sessions resume/abort/delete/export/import/share/unshare`, `runtime stop`, and `config`, and kept Phase 6 AgentProxy TUI explicitly not started. Verification passed: `git status --short`, `git log -1 --oneline`, `git diff --check`, and `pnpm run format:check`. Source code and tests were not changed.

## Current Iteration - 2026-05-21 Phase 5 Sessions Show CLI Minimal Workflow

Scope: advance only the Phase 5 `agentproxy sessions show <id>` read-only CLI task group. This workflow reads one AgentProxy local SQLite session index record for the selected provider/workspace and prints sanitized session details. Do not implement `sessions resume`, `sessions abort`, `sessions delete`, `sessions export`, `sessions import`, `sessions share`, `sessions unshare`, `runtime stop`, `config`, Phase 6 AgentProxy TUI, provider transcript display, live provider sync, or new Agent runtime behavior.

Implementation checklist:

- [x] Add focused CLI tests for `sessions show` human/JSON output, missing registry DB behavior, missing/tombstoned/wrong-provider/wrong-workspace records, disabled/invalid provider behavior, terminal/control-character safety, and later-command placeholder boundaries.
- [x] Extend the narrow `src/cli/sessions.ts` action/service layer to read a single existing session from SQLite with readonly/migrate:false/fileMustExist semantics and without creating storage when absent.
- [x] Wire only `agentproxy sessions show <id>` from planned placeholder to a real action; keep all mutating/session-runtime/config follow-up commands untouched.
- [x] Keep JSON output transcript-free and provider-payload-safe by emitting only stable local session index fields and optional sanitized event-count metadata, not raw event payloads or provider transcript content.
- [x] Update `docs/development-progress-tracker.zh.md` completed/unfinished status and Review notes after verification.
...
- Initial working tree is clean; `git log -1 --oneline` is `3c25a22 文档：同步 Phase 5 Sessions List 后续开发状态`, so the latest Phase 5 implementation baseline remains `6626584 阶段进展：完成 Phase 5 Sessions List CLI`.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.
- Phase 2.5 already provides SQLite `sessions.getById()` and `sessionEvents.listBySessionId()`.
- Phase 5 `sessions list` already provides the readonly SQLite visibility pattern, provider/workspace filtering expectations, JSON/human output split, and terminal/secret sanitization helpers.
- Phase 4 keeps provider transcript/content as provider source of truth; this iteration must remain a local index detail command and not sync or fetch live provider transcript.

Acceptance criteria for this iteration:

- [x] `agentproxy sessions show <id>` is a real read-only command and no longer returns the generic planned `CAPABILITY_UNSUPPORTED` placeholder.
- [x] When the AgentProxy SQLite registry DB is absent, `sessions show <id>` fails with stable `SESSION_NOT_FOUND` and does not create the DB or parent data directory.
- [x] Existing non-tombstoned sessions are shown only when the selected provider and workspace match the local session index record.
- [x] Tombstoned, wrong-provider, and wrong-workspace records are treated as not found for the default `show` command.
- [x] `sessions show <id> --json` emits exactly one valid redacted JSON object on stdout, with no human prose mixed in.
...
- [x] Non-OpenCode provider selection maps to `PROVIDER_NOT_FOUND`; disabled OpenCode config maps to `PROVIDER_UNAVAILABLE`.
- [x] `sessions resume/abort/delete/export/import/share/unshare`, `runtime stop`, `config`, `chat --session`, and Phase 6 AgentProxy TUI remain unimplemented or explicitly unsupported.
- [x] Focused tests pass, followed by `pnpm run typecheck`, `pnpm run test`, `pnpm run lint`, `pnpm run format:check`, `pnpm run build`, and `git diff --check`.

Risks and constraints:

- Do not contact OpenCode runtime or provider APIs from `sessions show`; live provider sync and transcript retrieval need separate design.
- Do not expose complete messages, provider raw payloads, event payload JSON, metadata blobs, or share/export URLs.
- Avoid adding migrations or schema fields; reuse existing local session and event repositories.
- Keep AgentProxy as a thin control plane and local index, not a replacement Agent runtime.

Review notes:

- 2026-05-21: Implemented the Phase 5 `sessions show` read-only CLI minimal workflow. Added focused tests first and confirmed the placeholder red state, then extended `src/cli/sessions.ts` and `src/cli/index.ts` so `agentproxy sessions show <id>` reads only the existing local SQLite session index with `readonly: true`, `migrate: false`, and `fileMustExist: true`. The command returns sanitized stable detail fields, treats missing DB/missing/tombstoned/wrong-provider/wrong-workspace rows as `SESSION_NOT_FOUND`, does not create storage when absent, sanitizes invalid provider ids in JSON errors, and leaves `sessions resume/abort/delete/export/import/share/unshare`, `runtime stop`, `config`, `chat --session`, and Phase 6 TUI untouched. Verification passed: `pnpm exec vitest run tests/cli-sessions.test.ts tests/cli-help.test.ts tests/cli-run.test.ts tests/cli-runtime.test.ts`, `pnpm run typecheck`, `pnpm run lint`, `pnpm run format:check`, `pnpm run test` (29 files, 223 tests), `pnpm run build`, and `git diff --check`. Code review found one documentation/todo status warning after full verification; this final Review update resolves it before commit. Residual risk: `sessions show` only shows the local AgentProxy session index and does not sync live provider sessions or read provider transcript.

## Current Iteration - 2026-05-21 Documentation Sync After Phase 5 Sessions List CLI

Scope: update tracking documents after `6626584 阶段进展：完成 Phase 5 Sessions List CLI`. This is a documentation-only synchronization so the next Codex session can continue from the correct first unfinished Phase 5 CLI MVP task group. Do not change source code, tests, provider behavior, runtime behavior, CLI command behavior, or TUI behavior.

Implementation checklist:

- [x] Confirm current git status and latest commit.
- [x] Update `docs/development-progress-tracker.zh.md` latest status, concrete implementation baseline, completed/unfinished summaries, Review entry, and next-start prompt.
- [x] Keep Phase 5 remaining CLI MVP work limited to `sessions show/resume/abort/delete/export/import/share/unshare`, `runtime stop`, and `config`; keep Phase 6 AgentProxy TUI explicitly not started.
- [x] Run documentation-appropriate verification.

Dependencies confirmed before implementation:

- Initial working tree is clean; `git log -1 --oneline` is `6626584 阶段进展：完成 Phase 5 Sessions List CLI`.
- Gate 4 validation baseline remains `549a979 阶段进展：完成 Gate 4 汇总验证`.
- Phase 5 implementation baseline should now be `6626584` unless a later documentation-only commit is on top.

Acceptance criteria:

- [x] The tracker explicitly states completed work through Phase 5 `sessions list`.
- [x] The tracker explicitly states Phase 5 is still in progress and `sessions show/resume/abort/delete/export/import/share/unshare`, `runtime stop`, and `config` remain unfinished.
- [x] The next-start prompt tells the next Codex session to verify git state and continue from one remaining Phase 5 CLI MVP task group, not Phase 6 TUI.
- [x] The tracker uses concrete commit `6626584` for the latest Phase 5 sessions list implementation baseline instead of a placeholder.
...
- 2026-05-21: Documentation sync completed after confirming a clean tree at `6626584`. Updated `docs/development-progress-tracker.zh.md` to use concrete Phase 5 sessions list implementation baseline `6626584`, kept Phase 5 remaining CLI MVP work limited to `sessions show/resume/abort/delete/export/import/share/unshare`, `runtime stop`, and `config`, and kept Phase 6 AgentProxy TUI explicitly not started. Verification passed: `git status --short`, `git log -1 --oneline`, `git diff --check`, and `pnpm run format:check`. Source code and tests were not changed.
